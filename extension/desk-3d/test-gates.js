...
assert.ok(html.includes('studio-desk'));
assert.ok(html.includes('id="window-lead"') && html.includes('这个窗口'));
assert.ok(html.includes('workset-save') && html.includes('>保存这个窗口<'));
assert.ok(html.includes('workset-restore-recent') && html.includes('恢复这'));
assert.ok(html.indexOf('id="resume"') < html.indexOf('id="desk-3d-mount"'));
assert.ok(html.indexOf('常用站') < html.indexOf('id="desk-3d-mount"'), 'mount sits after 常用站');
...
// The 3D stage is no longer stacked over the first screen, so the old
// resume z-index:12 / mount z-index:8 pair does not apply.
...
assert.ok(ui.includes('还没有存下的窗口'));
...
assert.ok(boot.includes('import('), 'embed.js is loaded dynamically');
assert.ok(!/import\s+[^;]*from\s+['"]\.\/embed\.js['"]/.test(boot), 'no static embed import');
assert.ok(boot.includes('sopify-spaceview'));
...
assert.ok(newtabJs.includes('getSpaceView'));
assert.ok(newtabJs.includes('spaceViewOn = false'), 'space view defaults off');
assert.ok(!/localStorage/.test(newtabJs), 'space view is not mirrored to localStorage');
