...
- **工作集**：只有点「保存这个窗口」才写入本机。最多 5 个；满了会问要不要覆盖最早的，不会悄悄丢掉。书桌可「恢复最近」；设置里列出全部，可删一条或「清空全部工作集」。恢复时打开当前窗口还没有的网址，已打开的同一网址就切过去。关窗口不会自动存。
...
`worksets` 形状：`[{ id, name, savedAt, tabs:[{ title, url }] }]`。不存 favicon。只收 http(s) 和 localhost。只在明确保存时写入。最多 5 个，可删、可清空。不用 `storage.sync`。
